...
- `docs/contributing.md` snippet-includes the root `CONTRIBUTING.md` (via `pymdownx.snippets`)
  rather than duplicating it. If you add a link inside `CONTRIBUTING.md`, use an absolute
  `https://github.com/Peppe37/tap/...` URL rather than a relative repo path — a relative path that
  resolves correctly from the repo root (where GitHub renders `CONTRIBUTING.md`) will not resolve
  from `docs/contributing.md`'s position in the MkDocs tree, and vice versa.
